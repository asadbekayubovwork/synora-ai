 ...
 *
 * Unlike `AppIcon` these are not one drawn set — each is the vendor's own
 * artwork on its own grid, so every logo keeps its native `viewBox` and only
 * the size comes from the class the caller puts on it. Google stays
 * multi-colour; GitHub and Telegram take `currentColor` so they read on either
 * theme.
 */
defineProps<{ provider: OAuthProviderName }>()
</script>

<template>
  <svg v-if="provider === 'google'" viewBox="0 0 18 18" aria-hidden="true">
  ...
  <svg v-else-if="provider === 'github'" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
